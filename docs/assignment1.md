...
> **Version 1.0.6** | CS336 Staff | Spring 2025
...
The tokenizer vocabulary is a one-to-one mapping from bytestring token to
...
Pre-tokenization
Once you have a vocabulary, you could, in principle, count how often bytes occur next
...
Compute BPE merges
Now that we’ve converted our input text into pre-tokens and represented each
...
> values.
> Pre-tokenization
> For simplicity and to focus on the merge procedure, we assume in this example
...
You will find that a major bottleneck is the pre-tokenization step. You
...
Before running pre-tokenization with the regex
...
counts, rather than explicitly iterating over each pair of bytes to count pair frequencies.
You can get
...
> in most regex engines and will be too slow in most that do.
> We have verified that Oniguruma is
...
> 3].
> Special tokens.
> Your tokenizer should be able to properly handle user-defined special tokens when encod-
...
bedding layer to produce a sequence of vectors.
...
3.3
Remark: Batching, Einsum and Eﬀicient Computation
...
We strongly recommend learning and using einsum notation for the class.
...
> You should set dff to approximately
> 8
...
> elements q(i)
> 2k−1:2k as 2d vectors by the angle θi,k =
> i
> Θ(2k−2)/d for k ∈{1, . . . , d/2} and some constant Θ. Thus,
...
Causal masking.
Your implementation should prevent the model from attending to future tokens in the
...
Applying RoPE.
RoPE should be applied to the query and key vectors, but not the value vectors. Also,
...
> to test your implementation.
> Perplexity
> Cross entropy suﬀices for training, but when we evaluate the model, we also want to report
...
> maximum value M, then we leave g as is; otherwise, we scale g down by a factor of
> M
...
m, paired with the corresponding next tokens, also with length m.
...
for any nn.optim.Optimizer.
Finally, torch.save(obj, dest) can dump an object (e.g., a dictionary
...
> like object), and then recover the model and optimizer states from that checkpoint.
> Your
> function should return the iteration number that was saved to the checkpoint.
> You can use
...
earlier in Eq 10.
Decoding
To generate text (decode) from our model, we will provide the model with a sequence of prefix
...
Decoder tricks
We will be experimenting with small models, and small models can sometimes generate
...
> We will tell you some very basic hyperparameters to start with and ask you to
...
3d_model while being a multiple of 64, which is good for GPU performance.
...
Putting it together
Now you can put everything together by getting a trained BPE tokenizer, tok-
...
Tips and tricks for debugging model architectures
We highly recommend getting comfortable with
...
> “Hey, Polly! Let’s go out!” said Tim. Sue looked at the sky and saw that it was diﬀicult to find a
...
> be roughly dff = 8
> 3dmodel (while ensuring that dff mod 64 = 0, to make use of GPU tensor cores). In your
...
You’re almost done!
You will now try to improve upon the
...
Runtime Your submission can run for at most 1.5 hours on an H100.
You can enforce this by setting
...
Transformers without tears: Improving the normalization of self-
...
Edouard Grave, and Guillaume Lample. Llama: Open and eﬀicient foundation language models, 2023.
arXiv:2302.13971.
